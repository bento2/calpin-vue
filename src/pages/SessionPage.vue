...
const { getSessionById, deleteSession, finishSession, restartSession, updateSession, saveSession } =
  useSessionStore()
...
<template>
  ...
  <div v-else class="h-100 d-flex flex-column">
    <div
      class="pa-2 d-flex align-center justify-space-between text-white bg-blue-accent-2 elevation-2"
    >
      ...
      <div class="text-h6 d-flex align-center">
        ...
        <AppBtn
          ...
          icon="mdi-pause"
          @click="menu = true"
          class="bg-white text-blue-accent-2"
        />
      </div>
    </div>
    ...
    <div class="overflow-y-auto flex-grow-1 px-2 pt-2 pb-14 bg-blue-accent-4 mb-2">
      <template v-for="(exercice, index) in session.exercices" :key="exercice.id">
        <SessionExerciceItem
          ...
          :is-last="index === session.exercices.length - 1"
          @move-up="moveUp"
          @move-down="moveDown"
          @remove="remove"
          @toggle="toggle"
    ...
    <!-- Footer/Finish -->
    <div
      class="pa-2 bg-white border-t ga-2"
      style="position: fixed; bottom: 0; left: 0; right: 0; z-index: 10"
    >
      <AppBtn
        block
        size="large"
        @click="dialogExercices = !dialogExercices"
