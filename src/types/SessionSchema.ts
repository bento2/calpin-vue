...
export const SessionSchema = z
  ...
  .transform((data) => ({
    ...
    get nbChecked() {
      return data.exercices.reduce(
        (count, currentValue) => (count += currentValue.completed ? 1 : 0),
        0,
      )
    },

    get total() {
      return data.exercices.reduce((count, currentValue) => count + currentValue.total, 0)
    },
  }))
